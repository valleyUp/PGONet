...
class PreNet(nn.Module):
    def __init__(self, n, sigmoid_n):
        ...
        self.ref_sol = torch.load('./case/SeaFloor/o_temp.pt').to(accelerator.device)
        self.ref_sol2 = torch.zeros((self.ref_sol.shape[0], 1, 3, 13)).to(accelerator.device)
    ...
    def get_local_Loss(self, res):
        res1 = res.clone()
        local_res = torch.zeros((res.shape[0], 1, 3, 13)).to(accelerator.device)
        local_res[:, :, 0, 0] = res1[:, :, int(37.5 * self.num), 2 * self.num]
        local_res[:, :, 0, 1] = res1[:, :, int(37.5 * self.num), 7 * self.num]
        local_res[:, :, 0, 2] = res1[:, :, int(37.5 * self.num), 12 * self.num]
        local_res[:, :, 0, 3] = res1[:, :, int(37.5 * self.num), 17 * self.num]
        local_res[:, :, 0, 4] = res1[:, :, int(37.5 * self.num), 22 * self.num]
        local_res[:, :, 0, 5] = res1[:, :, int(37.5 * self.num), 27 * self.num]
        local_res[:, :, 0, 6] = res1[:, :, int(37.5 * self.num), 32 * self.num]
        local_res[:, :, 0, 7] = res1[:, :, int(37.5 * self.num), 37 * self.num]
        local_res[:, :, 0, 8] = res1[:, :, int(37.5 * self.num), 42 * self.num]
        local_res[:, :, 0, 9] = res1[:, :, int(37.5 * self.num), 47 * self.num]
        local_res[:, :, 0, 10] = res1[:, :, int(37.5 * self.num), 52 * self.num]
        local_res[:, :, 0, 11] = res1[:, :, int(37.5 * self.num), 57 * self.num]
        local_res[:, :, 0, 12] = res1[:, :, int(37.5 * self.num), 62 * self.num]

        local_res[:, :, 2, 0] = res1[:, :, int(35 * self.num), 2 * self.num]
        local_res[:, :, 2, 1] = res1[:, :, int(35 * self.num), 7 * self.num]
        local_res[:, :, 2, 2] = res1[:, :, int(35 * self.num), 12 * self.num]
        local_res[:, :, 2, 3] = res1[:, :, int(35 * self.num), 17 * self.num]
        local_res[:, :, 2, 4] = res1[:, :, int(35 * self.num), 22 * self.num]
        local_res[:, :, 2, 5] = res1[:, :, int(35 * self.num), 27 * self.num]
        local_res[:, :, 2, 6] = res1[:, :, int(35 * self.num), 32 * self.num]
        local_res[:, :, 2, 7] = res1[:, :, int(35 * self.num), 37 * self.num]
        local_res[:, :, 2, 8] = res1[:, :, int(35 * self.num), 42 * self.num]
        local_res[:, :, 2, 9] = res1[:, :, int(35 * self.num), 47 * self.num]
        local_res[:, :, 2, 10] = res1[:, :, int(35 * self.num), 52 * self.num]
        local_res[:, :, 2, 11] = res1[:, :, int(35 * self.num), 57 * self.num]
        local_res[:, :, 2, 12] = res1[:, :, int(35 * self.num), 62 * self.num]

        local_res[:, :, 1, 0] = res1[:, :, 45 * self.num, 7 * self.num]
        local_res[:, :, 1, 1] = res1[:, :, 45 * self.num, 10 * self.num]
        local_res[:, :, 1, 2] = res1[:, :, 45 * self.num, 13 * self.num]
        local_res[:, :, 1, 3] = res1[:, :, 45 * self.num, 16 * self.num]
        local_res[:, :, 1, 4] = res1[:, :, 45 * self.num, 19 * self.num]
        local_res[:, :, 1, 5] = res1[:, :, 45 * self.num, 22 * self.num]
        local_res[:, :, 1, 6] = res1[:, :, 45 * self.num, 25 * self.num]
        local_res[:, :, 1, 7] = res1[:, :, 45 * self.num, 28 * self.num]
        local_res[:, :, 1, 8] = res1[:, :, 45 * self.num, 31 * self.num]
        local_res[:, :, 1, 9] = res1[:, :, 45 * self.num, 34 * self.num]
        local_res[:, :, 1, 10] = res1[:, :, 45 * self.num, 37 * self.num]
        local_res[:, :, 1, 11] = res1[:, :, 45 * self.num, 58 * self.num]
        local_res[:, :, 1, 12] = res1[:, :, 45 * self.num, 62 * self.num]
...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/SeaFloor/o_temp.pt').to(accelerator.device)
        self.test_ref_speed = torch.load('./case/SeaFloor/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).to(accelerator.device)
...
def compute_loss_p(model, output71, loss_func, ntb, ref_speed, bsize, last_loss_weight, size_batch, last_ref_speed, coffe, epoch):
    ...
    mse_loss = nn.MSELoss(reduction='mean')
    x1 = np.load('./case/SeaFloor/x1.npy')
    y1 = np.load('./case/SeaFloor/y1.npy')
...
def train(model, model1, input, n_iters, n_iters1, n_iters2, time_batch_size,
    ...
    # ref_speed = torch.load('./case/SeaFloor/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0)
    ...
    for epoch in range(n_iters):
        ...
        for epoch1 in range(n_iters1):
            ...
            if tt_flag:
                torch.save(input, './res/SeaFloor/tensor_'+ str(epoch)+"_" + str(epoch1) + '.pt')
                tt_flag = False
                break
        ...
        for epoch2 in range(n_iters2):
            ...
            if (epoch2+1) % 50 == 0:
                ...
                plt.colorbar()
                #plt.show()
                plt.savefig('./res/SeaFloor/fig/speed_' + str(epoch) + "_" + str(epoch2) + '.png')
                plt.close()
                torch.save(ref_speed, './res/SeaFloor/ref_speed/speed_' + str(epoch)+"_" + str(epoch2) +  '.pt')
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/SeaFloor/o_temp.pt")
    ...
    np.save('./res/SeaFloor/train_loss', train_loss)
