...
class PreNet(nn.Module):
    def __init__(self, n, sigmoid_n):
        ...
        self.ref_sol = torch.load('./case/SeaFloor/o_temp.pt').cuda()
    ...
    def get_local_Loss(self, res):
        res1 = res.clone()
        local_res = torch.zeros((res.shape[0], 1, 3, 13)).cuda()
...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/SeaFloor/o_temp.pt').cuda()
        self.test_ref_speed = torch.load('./case/SeaFloor/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).cuda()
