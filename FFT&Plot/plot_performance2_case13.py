...
for idx in range(14):
    ...
    if idx < 7:  # 第二行
        ...
        print((indices[idx-7])*float(1/25)," ",F.mse_loss(file2[indices[idx]].cuda(),file1[indices[idx]].cuda()))
    elif idx < 14:  # 第二行
        img = (file3[indices[idx-7]].cuda()-file1[indices[idx-7]].cuda()).detach().cpu().numpy()
        title = f'PINN, {(indices[idx-14])*float(1/25):.3f}s'
        print((indices[idx-14])*float(1/25)," ",F.mse_loss(file3[indices[idx-7]].cuda(),file1[indices[idx-7]].cuda()))
