...
for idx in range(28):
    ...
    if idx < 7:  # 第二行
        ...
        print((indices[idx])*float(1/25)," ",F.mse_loss(file2[indices[idx]].cuda(),file1[indices[idx]].cuda()))
    elif idx < 14:  # 第二行
        img = (file3[indices[idx-7]].cuda()-file1[indices[idx-7]].cuda()).detach().cpu().numpy()
        title = f'PGONet \n without RC, {(indices[idx-7])*float(1/25):.3f}s'
        print((indices[idx-7])*float(1/25)," ",F.mse_loss(file3[indices[idx-7]].cuda(),file1[indices[idx-7]].cuda()))
    elif idx < 21:  # 第二行
        img = (file4[indices[idx-14]].cuda()-file1[indices[idx-14]].cuda()).detach().cpu().numpy()
        title = f'PINN, {(indices[idx-14])*float(1/25):.3f}s'
        print((indices[idx-14])*float(1/25)," ",F.mse_loss(file4[indices[idx-14]].cuda(),file1[indices[idx-14]].cuda()))
    elif idx < 28:  # 第二行
        img = (file5[indices[idx-21]].cuda()-file1[indices[idx-21]].cuda()).detach().cpu().numpy()
        title = f'UNet, {(indices[idx-21])*float(1/25):.3f}s'
        print((indices[idx-21])*float(1/25)," ",F.mse_loss(file5[indices[idx-21]].cuda(),file1[indices[idx-21]].cuda()))
