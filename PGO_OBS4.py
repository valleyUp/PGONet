...
class PreNet(nn.Module):
    def __init__(self, n, sigmoid_n):
        ...
        self.ref_sol = torch.load('./case/MultiObs3/o_temp.pt').to(accelerator.device)
...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/MultiObs3/o_temp.pt').to(accelerator.device)
        self.test_ref_speed = torch.load('./case/MultiObs3/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).to(accelerator.device)
