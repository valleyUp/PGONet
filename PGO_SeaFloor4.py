...
class PreNet(nn.Module):
    ...
    def get_local_Loss(self, res):
        ...
        local_res[:, :, 0, 4] = res1[:, :, int(37.5 * self.num), 32 * self.num]
        local_res[:, :, 0, 5] = res1[:, :, int(37.5 * self.num), 40 * self.num]
        local_res[:, :, 0, 6] = res1[:, :, int(37.5 * self.num), 48 * self.num]
        local_res[:, :, 0, 7] = res1[:, :, int(37.5 * self.num), 56 * self.num]
        local_res[:, :, 0, 8] = res1[:, :, int(37.5 * self.num), 62 * self.num]

        local_res[:, :, 2, 0] = res1[:, :, int(32.5 * self.num), 2 * self.num]
        local_res[:, :, 2, 1] = res1[:, :, int(32.5 * self.num), 8 * self.num]
        local_res[:, :, 2, 2] = res1[:, :, int(32.5 * self.num), 16 * self.num]
        local_res[:, :, 2, 3] = res1[:, :, int(32.5 * self.num), 24 * self.num]
        local_res[:, :, 2, 4] = res1[:, :, int(32.5 * self.num), 32 * self.num]
        local_res[:, :, 2, 5] = res1[:, :, int(32.5 * self.num), 40 * self.num]
        local_res[:, :, 2, 6] = res1[:, :, int(32.5 * self.num), 48 * self.num]
        local_res[:, :, 2, 7] = res1[:, :, int(32.5 * self.num), 56 * self.num]
        local_res[:, :, 2, 8] = res1[:, :, int(32.5 * self.num), 62 * self.num]

        local_res[:, :, 1, 0] = res1[:, :, 45 * self.num, int(2.5 * self.num)]
        local_res[:, :, 1, 1] = res1[:, :, 45 * self.num, int(5 * self.num)]
        local_res[:, :, 1, 2] = res1[:, :, 45 * self.num, int(7.5 * self.num)]
        local_res[:, :, 1, 3] = res1[:, :, 45 * self.num, int(10 * self.num)]
        local_res[:, :, 1, 4] = res1[:, :, 45 * self.num, int(12.5 * self.num)]
        local_res[:, :, 1, 5] = res1[:, :, 45 * self.num, int(53 * self.num)]
        local_res[:, :, 1, 6] = res1[:, :, 45 * self.num, int(56 * self.num)]
        local_res[:, :, 1, 7] = res1[:, :, 45 * self.num, int(59 * self.num)]
        local_res[:, :, 1, 8] = res1[:, :, 45 * self.num, int(62 * self.num)]
...
def train(model, model1, input, n_iters, n_iters1, n_iters2, time_batch_size,
    ...
    for epoch in range(n_iters):
        ...
        for epoch1 in range(n_iters1):
            ...
            if tt_flag:
                torch.save(input, './res/SeaFloor3/tensor_'+ str(epoch)+"_" + str(epoch1) + '.pt')
                tt_flag = False
                break
        ...
        for epoch2 in range(n_iters2):
            ...
            if (epoch2+1) % 50 == 0:
                ...
                plt.colorbar()
                #plt.show()
                plt.savefig('./res/SeaFloor3/fig/speed_' + str(epoch) + "_" + str(epoch2) + '.png')
                plt.close()
                torch.save(ref_speed, './res/SeaFloor3/ref_speed/speed_' + str(epoch)+"_" + str(epoch2) +  '.pt')
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/SeaFloor3/o_temp.pt")
    ...
    np.save('./res/SeaFloor3/train_loss', train_loss)
