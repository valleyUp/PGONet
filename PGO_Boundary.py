...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.test_ref_speed = torch.load('./case/Boundary/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).cuda()
    ...
    def forward(self, ref_speed, bsize, id, loc_x, loc_y, batch,
        ...
        x_temp7[:, :, -1, :] = 0
        if (step + id * bsize + 2) * dt < 0.01:
            for idx in range(len(loc_x)):
                if loc_x[idx] != -1:
                    x_temp7[:, :, int(loc_x[idx]), int(loc_y[idx])] = 1500  # * np.sin(2 * 3.1415926 * self.fre * (bsize * id + step + 2) * dt)
        else:
            for idx in range(len(loc_x)):
                if loc_x[idx] != -1:
                    x_temp7[:, :, int(loc_x[idx]),
                    int(loc_y[idx])] = 0
    ...
    for epoch in range(n_iters):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # with torch.autograd.set_detect_anomaly(True):
                ...
                loss, loss_local2, loss_local3, loss_true,temp_num, input, t_flag, t_epoch, x_tt, x_t=compute_loss(output1, output2, output3,loss_func,
        ...
        if epoch % 50000 == 0 and epoch != 0:
            torch.save(input, './res/tensor_boundary/tensor_' + str(epoch) + '_' + str(step) + '.pt')
        t_epoch += 1
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/Boundary/o_temp.pt")
    ...
    np.save('./res/tensor_boundary/train_loss', train_loss)
