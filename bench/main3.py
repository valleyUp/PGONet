...
class LossFunctions:
    ...
    def pde(self, X, X_i1):
        ...
        y = X[:, 1:2]
        t = X[:, 2:3]
        X_i = torch.reshape(X_i1, (1,X_i1.shape[0])).cuda()
        u = self.model(X_i,torch.cat([x, y, t], 1))
        u_x = self.derive(u, x)
    ...
    def reconstruction(self, X, Y):
        ...
        """
        X_i = torch.reshape(Y, (1, Y.shape[0])).cuda()
        u = self.model(X_i, X)
        recon_loss = u - X_i
...
class Training:
    ...
    def training_loop(self):
        ...
        X_i = torch.reshape(self.Y_i, (1,self.Y_i.shape[0])).cuda()
        while it < epochs:

            self.optimiser.zero_grad()
            initial_loss = self.loss_fnc.reconstruction(self.X_i, self.Y_i)
            boundary_loss = self.loss_fnc.boundary(self.X_b, self.Y_i)
            domain_loss = self.loss_fnc.pde(self.X_f, self.Y_i)
            ...
            if it % 10000 == 0:
                with torch.no_grad():
                    u_pred = self.model(X_i,self.config_obj.torch_tensor_grad(self.X_star,
                ...
                torch.save(outputs1, './u_res2_'+str(it)+'_DeepONet.pt')
...
if __name__ == "__main__":
    ...
    model = deeponet.DeepONet(branch_size=126*126,
                 trunk_size=3,
                 hidden_size=64,
                 num_layers=5,
                 activation="tanh",
