...
for i in range(1):
    for j in range(126):
        for k in range(126):
            ...
            x_i[i * 126 * 126 + j * 126 + k, 0] = (j+1)
            x_i[i * 126 * 126 + j * 126 + k, 1] = (k+1)
...
x_f = torch.ones((63*63* 32, 3)).cuda()
true_f = torch.ones((63*63* 32, 1)).cuda()
for i in range(32):
    for j in range(63):
        for k in range(63):
            x_f[i * 63*63 + j * 63 + k, 2] = (i + 1) * 2 * float(1/25)
    ...
    for j in range(128):
        for k in range(4):
            ...
            if k == 0:
                ...
                x_b[i * 128 * 4 + j * 4 + k, 1] = j
            if k == 1:
                x_b[i * 128 * 4 + j * 4 + k, 0] = 127
                x_b[i * 128 * 4 + j * 4 + k, 1] = j
            if k == 2:
                x_b[i * 128 * 4 + j * 4 + k, 0] = j
                x_b[i * 128 * 4 + j * 4 + k, 1] = 0
            if k == 3:
                x_b[i * 128 * 4 + j * 4 + k, 0] = j
                x_b[i * 128 * 4 + j * 4 + k, 1] = 127
