...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/Performance/o_temp.pt').to(accelerator.device)
        self.test_ref_speed = torch.load('./case/Performance/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).to(accelerator.device)
        self.apply(initialize_weights)

    def forward(self, ref_speed, bsize, batch,
        ...
        #x_t41 = self.input_layer7(torch.concat((x_tt,x_t,ref_speed),dim=1))
...
def train(model, input, n_iters, time_batch_size,
    ...
    x1 = np.load('./case/Performance/x1.npy')
    y1 = np.load('./case/Performance/y1.npy')
    ...
    ref_speed = torch.load('./case/Performance/ref_speed.pt').to(accelerator.device)
    ...
    for epoch in range(n_iters):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            for time_batch_id in range(flag_num[step]-1,flag_num[step],1):
                if time_batch_id == 0 and temp_num-1 == 0:
                    ...
                    x_tt = batch[size_batch*time_batch_id:size_batch*time_batch_id+1, :, :, :].detach()
                    x_t = batch[size_batch*time_batch_id+1:size_batch*time_batch_id+2, :, :, :].detach()

                # output is a list
                ref_speed = ref_speed.detach()
                output4, output1, output2, output3, second_last_state_forward, test_speed \
                        = model(ref_speed, size_batch, input, flag_num[step], temp_num, x_tt, x_t)
        ...
        if epoch % 50000 == 0 and epoch != 0:
            torch.save(input, './res/Performance/tensor_' + str(epoch) + '_' + str(step) + '.pt')
        t_epoch += 1
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/Performance/o_temp.pt")
    ...
    np.save('./res/Performance/train_loss', train_loss)
