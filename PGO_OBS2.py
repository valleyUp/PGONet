...
class PreNet(nn.Module):
    def __init__(self, n, sigmoid_n):
        ...
        self.ref_sol = torch.load('./case/MultiObs1/o_temp.pt').to(accelerator.device)
...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/MultiObs1/o_temp.pt').to(accelerator.device)
        self.test_ref_speed = torch.load('./case/MultiObs1/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).to(accelerator.device)
...
def compute_loss_p(model, output71, loss_func, ntb, ref_speed, bsize, last_loss_weight, size_batch, last_ref_speed, coffe, epoch):
    ...
    mse_loss = nn.MSELoss(reduction='mean')
    x1 = np.load('./case/MultiObs1/x1.npy')
    y1 = np.load('./case/MultiObs1/y1.npy')
    ref_local_sol = loss_func.get_ref_Loss().to(accelerator.device)
...
def train(model, model1, input, n_iters, n_iters1, n_iters2, time_batch_size,
    ...
    for epoch in range(n_iters):
        ...
        for epoch1 in range(n_iters1):
            ...
            if tt_flag:
                torch.save(input, './res/MultiObs1/tensor_'+ str(epoch)+"_" + str(epoch1) + '.pt')
                tt_flag = False
                break
        ...
        for epoch2 in range(n_iters2):
            ...
            if (epoch2+1) % 50 == 0:
                ...
                plt.colorbar()
                #plt.show()
                plt.savefig('./res/MultiObs1/fig/speed_' + str(epoch) + "_" + str(epoch2) + '.png')
                plt.close()
                torch.save(ref_speed, './res/MultiObs1/ref_speed/speed_' + str(epoch)+"_" + str(epoch2) +  '.pt')
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/MultiObs1/o_temp.pt")
    ...
    np.save('./res/MultiObs1/train_loss', train_loss)
