...
class PreNet(nn.Module):
    def __init__(self, n, sigmoid_n):
        ...
        self.ref_sol = torch.load('./case/MultiObs1/o_temp.pt').cuda()
...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/MultiObs1/o_temp.pt').cuda()
        self.test_ref_speed = torch.load('./case/MultiObs1/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).cuda()
    ...
    for id in range(ntb):
        for idx in range(num_time_batch2):
            ...
            if idx!=0:
                output714[0:1] = x_tt1
                output714[1:2] = x_t1
            output11_3 = loss_func.get_phy_Loss1(output714.clone(), ref_speed, size_batch, idx, x1[id], y1[id], 1)
...
if __name__ == '__main__':
    ...
    # local_res = torch.zeros((input_tensor.shape[0], 1, 20, 20)).cuda()
    # for x in range(20):
    #     for y in range(20):
    #         local_res[:, :, x, y] = res1[:, :, 2 * (x + 1), 3 * (y + 1)]
    #input_tensor2 = torch.load("./case/o_temp.pt")
    #input2 = torch.zeros((2,1,128,128)).cuda()
    #input2[0:2,:,:,:]=input_tensor[0:2,:,:,:]
