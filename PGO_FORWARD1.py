...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/Forward/o_temp.pt').cuda()
        self.test_ref_speed = torch.load('./case/Forward/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).cuda()
...
def train(model, input, n_iters, time_batch_size,
    ...
    ref_speed = torch.load('./case/Forward/ref_speed.pt').cuda()
