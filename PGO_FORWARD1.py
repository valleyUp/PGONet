...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/Forward/o_temp.pt').to(accelerator.device)
        self.test_ref_speed = torch.load('./case/Forward/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).to(accelerator.device)
...
def train(model, input, n_iters, time_batch_size,
    ...
    x1 = np.load('./case/Forward/x1.npy')
    y1 = np.load('./case/Forward/y1.npy')
    ...
    ref_speed = torch.load('./case/Forward/ref_speed.pt').to(accelerator.device)
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/Forward/o_temp.pt")
