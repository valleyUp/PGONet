...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/Forward1/o_temp.pt').cuda()
        self.test_ref_speed = torch.load('./case/Forward1/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).cuda()
...
def train(model, input, n_iters, time_batch_size,
    ...
    ref_speed = torch.load('./case/Forward1/ref_speed.pt').cuda()
    ...
    for epoch in range(n_iters):
        ...
        if epoch % 50000 == 0 and epoch != 0:
            torch.save(input, './res/tensor_forward1/tensor_' + str(epoch) + '_' + str(step) + '.pt')
        t_epoch += 1
...
if __name__ == '__main__':
    ...
    np.save('./res/tensor_forward1/train_loss', train_loss)
