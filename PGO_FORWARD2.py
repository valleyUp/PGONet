...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/Forward1/o_temp.pt').to(accelerator.device)
        self.test_ref_speed = torch.load('./case/Forward1/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).to(accelerator.device)
...
def train(model, input, n_iters, time_batch_size,
    ...
    x1 = np.load('./case/Forward1/x1.npy')
    y1 = np.load('./case/Forward1/y1.npy')
    ...
    ref_speed = torch.load('./case/Forward1/ref_speed.pt').to(accelerator.device)
    ...
    for epoch in range(n_iters):
        ...
        if epoch % 50000 == 0 and epoch != 0:
            torch.save(input, './res/Forward1/tensor_' + str(epoch) + '_' + str(step) + '.pt')
        t_epoch += 1
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/Forward1/o_temp.pt")
    ...
    np.save('./res/Forward1/train_loss', train_loss)
