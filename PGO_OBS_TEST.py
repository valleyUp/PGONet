...
class PreNet(nn.Module):
    def __init__(self, n, sigmoid_n):
        ...
        self.ref_sol = torch.load('./case/MultiObs/o_temp.pt').to(accelerator.device)
        self.ref_sol2 = torch.zeros((self.ref_sol.shape[0], 1, 4*self.num, 4*self.num)).to(accelerator.device)
    ...
    def get_local_Loss(self, res):
        ...
        local_res = torch.zeros((res1.shape[0], 1, 4 * n, 4 * n)).to(accelerator.device)
        for x in range(4 * n):
            for y in range(4 * n):
    ...
    def forward(self, output, bsize, ntb):
        ...
        ref_2 = torch.zeros((1, 1, 64 * self.num, 64 * self.num)).to(accelerator.device)
...
class PGONet(nn.Module):
    ...
    def __init__(self, dt, dx, fre):
        ...
        self.ref_sol = torch.load('./case/MultiObs/o_temp.pt').to(accelerator.device)
        self.test_ref_speed = torch.load('./case/MultiObs/ref_speed.pt').unsqueeze(dim=0).unsqueeze(dim=0).to(accelerator.device)
...
class loss_generator(nn.Module):
    ...
    def get_phy_Loss2(self, model, output, c, bsize1, id2, loc_x, loc_y, coffe):
        ...
        output3 = torch.zeros_like(output1[:2+int(coffe*bsize1), :, :]).to(accelerator.device)
...
def compute_loss_p(model, output71, loss_func, ntb, ref_speed, bsize, last_loss_weight, size_batch, last_ref_speed, coffe, epoch):
    ...
    mse_loss = nn.MSELoss(reduction='mean')
    x1 = np.load('./case/MultiObs/x1.npy')
    y1 = np.load('./case/MultiObs/y1.npy')
    ref_local_sol = loss_func.get_ref_Loss().to(accelerator.device)
    ...
    factor = (1+mse_loss(ref_speed[:,:,:,:], 1500 * torch.ones(1,1,128,128).to(accelerator.device))/(1160**2))
    output_t = None
    x_tt1 = None
    x_t1 = None
    p_local = 0
    ...
    for id in range(ntb):
        for idx in range(num_time_batch2):
            ...
            p_sim +=  mse_loss(output7[2:, :, :, :].to(accelerator.deivce), output11_3)
...
def train(model, model1, input, n_iters, n_iters1, n_iters2, time_batch_size,
    ...
    for epoch in range(n_iters):
        ...
        for epoch1 in range(n_iters1):
            ...
            if tt_flag:
                torch.save(input, './res/MultiObs/tensor_'+ str(epoch)+"_" + str(epoch1) + '.pt')
                tt_flag = False
                break
        ...
        for epoch2 in range(n_iters2):
            ...
            if (epoch2+1) % 50 == 0:
                ...
                plt.colorbar()
                #plt.show()
                plt.savefig('./res/MultiObs/fig/speed_' + str(epoch) + "_" + str(epoch2) + '.png')
                plt.close()
                torch.save(ref_speed, './res/MultiObs/ref_speed/speed_' + str(epoch)+"_" + str(epoch2) +  '.pt')
...
if __name__ == '__main__':
    input_tensor = torch.load("./case/MultiObs/o_temp.pt")
    ...
    np.save('./res/MultiObs/train_loss', train_loss)
